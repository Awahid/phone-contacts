...
- (void)addContactName:(NSString *)firstName lastName:(NSString *)lastName phones:(NSArray *)phonesList emails:(NSArray *)emailsList birthday:(NSDate *)birthday image:(UIImage *)image completion:(CompletionBlock) completionBlock
{
    CFErrorRef *error = nil;
    ABAddressBookRef addressBook = ABAddressBookCreateWithOptions(nil, error);
    ...
    ABRecordID contactID = ABRecordGetRecordID(record);
    NSString* identifier = [NSString stringWithFormat:@"%d", contactID];
    
    
    completionBlock(wasAdded, identifier);
}

- (void)updateContact:(NSInteger)contactID firstName:(NSString *)firstName lastName:(NSString *)lastName phones:(NSArray *)phonesList emails:(NSArray *)emailsList completion:(CompletionBlock) completionBlock{
    ...
    NSString* identifier = [NSString stringWithFormat:@"%d", recordID];
    
    completionBlock(true, identifier);
}
